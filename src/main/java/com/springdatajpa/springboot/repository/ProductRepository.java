...
public interface ProductRepository extends JpaRepository<Product,Long> {
     ...
     /** NOTE:
     * Hibernate has pre defined naming function OR, AND etc
     ...
     * */
     ...
     /**Define JPQL query using @Query annotation with index or position parameters 1,2 ...
      * here Product refers to entity class not table and 'p' is alias to refer.
      */

     //with index or position parameters 1,2

     @Query("select p from Product p where p.name = ?1 and p.price =?2")
     List<Product> findByNameAndDescriptionJPQL(String name, BigDecimal price);

     //with naming parameters not index

     @Query("select p from Product p where p.name = :pName or p.price = :pPrice")
     List<Product> findByNameAndDescriptionNamingJPQL(@Param("pName") String name,
                                                      @Param("pPrice") BigDecimal price);
